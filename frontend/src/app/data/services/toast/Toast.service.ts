...
export class ToastService extends SignalsStoreService<ToastProps>{
  ...
  public info(title: string, message: string, seconds: number, position: ToastPosition = ToastPosition.TOP_RIGHT): void{
    this.show(title, message, seconds, ToastType.INFO, position);
  }

  public success(title: string, message: string, seconds: number, position: ToastPosition = ToastPosition.TOP_RIGHT): void{

    this.show(title, message, seconds, ToastType.SUCCESS, position);

  }

  public waring(title: string, message: string, seconds: number, position: ToastPosition = ToastPosition.TOP_RIGHT): void{

    this.show(title, message, seconds, ToastType.WARNING, position);
  }

  public error(title: string, message: string, seconds: number, position: ToastPosition = ToastPosition.TOP_RIGHT): void{
